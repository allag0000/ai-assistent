    ...
    <div className="flex flex-col h-full bg-slate-950 text-slate-200 font-sans select-none" dir="rtl">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-20 glass-effect border-t border-white/5 flex items-center justify-around px-2 pb-2 z-30">
        ...
        >
          ...
          <span className="text-[8px] font-bold">المحادثة</span>
        </button>
        ...
        >
          ...
          <span className="text-[8px] font-bold">الرندر</span>
        </button>
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 transition-all duration-300 ${activeTab === 'plans' ? 'text-indigo-400 scale-110' : 'text-slate-500'}`}
        >
          <div className={`p-2 rounded-xl transition-colors ${activeTab === 'plans' ? 'bg-indigo-500/10' : ''}`}>
            <PlanIcon size={18} />
          </div>
          <span className="text-[8px] font-bold">مخططات</span>
        </button>

        <button 
          onClick={() => setActiveTab('3dfactory')}
          className={`flex flex-col items-center gap-1 transition-all duration-300 ${activeTab === '3dfactory' ? 'text-indigo-400 scale-110' : 'text-slate-500'}`}
        >
          <div className={`p-2 rounded-xl transition-colors ${activeTab === '3dfactory' ? 'bg-indigo-500/10' : ''}`}>
            <BoxIcon size={18} />
          </div>
          <span className="text-[8px] font-bold">مصنع 3D</span>
        </button>
      </nav>
    </div>
  );
